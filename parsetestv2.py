...
class ValveDict(dict):
    ...
    def __setitem__(self, key, value):
        vmf_key = 'vmf_%s' % key
        assert hasattr(self, vmf_key), "Illigal Key %s" % key
        allowedcontainer = getattr(self, vmf_key)
        assert type(value) is allowedcontainer, \
        "Illigal Container %s for key `%s`, expected: %s" % (
            type(value), key, allowedcontainer
        )
